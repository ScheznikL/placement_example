...
fun RegistrationScreen(
    ...
    val wrongPasswordError by viewModel.wrongPasswordError.collectAsState()

    val isConfirmPasswordError by remember { viewModel.isConfirmPasswordError }

    var hidePassword by remember { mutableStateOf(true) }
    var register by remember { viewModel.isRegister }
        ...
        Column(
            ...
            Column(
                ...
                OutlinedTextField(modifier = Modifier.padding(5.dp),
                    ...
                    isError = if (register) isPasswordError else wrongPasswordError == true,
                    value = password,
                    onValueChange = {
                    ...
                    supportingText = {
                        if (isPasswordError && register) Text(text = stringResource(R.string.password_requirements))
                        else if (wrongPasswordError == true && !register) {
                            Text(text = stringResource(R.string.wrong_password_error))
                        }
                    },
                ...
                Button(enabled = if (register) !isPasswordError && !isConfirmPasswordError && !isEmailError && email.isNotBlank() && password.isNotBlank() else email.isNotBlank() && password.isNotBlank(),
                    onClick = {
                        if (register) {
                            viewModel.onSignUp()
                        } else {
